...
from transformers import HubertForSequenceClassification, WavLMForSequenceClassification, \
    UniSpeechSatForSequenceClassification, Wav2Vec2ForSequenceClassification
import torch
import torch.nn as nn
...
from KNNrescale import compute_weighted_knn_scores
...
def apply_specaug(spec, max_time_mask=80, max_freq_mask=80, num_time_masks=1, num_freq_masks=1):
    """
    对输入的频谱图应用 SpecAug 增强。

    参数:
    - spec: 输入的频谱图，形状为 (T, F) 或 (1, T, F)，其中 T 是时间步数，F 是频率通道数。
    - max_time_mask: 最大时间遮罩长度，默认为 80。
    - max_freq_mask: 最大频率遮罩长度，默认为 80。
    - num_time_masks: 应用的时间遮罩的数量，默认为 1。
    - num_freq_masks: 应用的频率遮罩的数量，默认为 1。

    返回:
    - augmented_spec: 经过 SpecAug 增强的频谱图。
    """

    # 如果输入是 2D 张量，添加一个 batch 维度 (1, T, F)
    if len(spec.shape) == 2:
...
class BEATsWithNewLayer(BEATs):
    def __init__(self, cfg, num_labels):
        ...
        # 不用额外的fc层，用arcface做分类映射
        self.arcface = ArcMarginProduct(in_features=768 * 2, out_features=num_labels)

    def forward(self, source, label=None, padding_mask=None):
        x, _, lprobs = self.extract_features(source, padding_mask, need_weights=True, layer=11)
        utterance_embedding = self.asp_layer(x)  # 形状: [batch, 768*2]

        if label is not None:
            logits = self.arcface(utterance_embedding, label)
        else:
            # 预测时不带标签，ArcFace通常需要label参与计算，否则直接用cosine或fc层
            logits = self.arcface(utterance_embedding, None)

        return logits, utterance_embedding
...
def train(model, train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, label) in enumerate(train_loader):
        ...
        loss = loss_function(outputs[0], label)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        # 每 n 次 steps 更新一次参数
...
dev_data_path = r"F:\DCASE\dcase2023t2\data\dcase2024t2\dev_data\raw"
eval_data_path = r"F:\DCASE\dcase2023t2\data\dcase2024t2\eval_data\raw"
...
for k_ensemble in np.arange(idx, idx + 5):
    ...
    model = BEATsWithNewLayer(cfg, num_label)
    ...
    for k in np.arange(aeons):
        ...
        print('aeon: ' + str(k + 1))
        # fit model2025
        weight_path = "F:/dcasemodel/2024/fullmodel/" + 'wts_' + str(k + 1) + 'k_' + str(target_sr) + '_' + str(
        ...
        with torch.no_grad():
            ...
            test_embs = []
            unknown_embs = []
            label = np.zeros((batch_size, num_label))
